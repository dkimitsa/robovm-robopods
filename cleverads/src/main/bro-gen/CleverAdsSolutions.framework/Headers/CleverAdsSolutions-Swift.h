...
typedef SWIFT_ENUM_NAMED(NSInteger, CASSourceId, "AdSourceId", open) {
  ...
  CASSourceIdYangoAds = 15,
  CASSourceIdHyprMX = 16,
  CASSourceIdSmaato = 18,
  CASSourceIdBigo = 19,
  CASSourceIdOgury = 20,
  CASSourceIdMadex = 21,
  CASSourceIdMonetrix = 22,
...
+ (void)reportCustomRevenueWithJson:(NSString * _Nonnull)json;
...
/// Should be set before <code>CASNativeView.bindAdContent(_:)</code>
...
/// You should register the native ad object with the view responsible for displaying it using <code>CASNativeView.bindAdContent</code>.
...
/// The media view will be populated automatically when calling <code>CASNativeView.bindAdContent</code>.
...
///   </li>
///   <li>
///     Setting the same template size multiple times does not regenerate the view hierarchy.
///   </li>
///   <li>
///     Use <code>bindAdContent</code> each time the template size is changed.
///   </li>
///   <li>
///     Use <code>bindAdContent</code> for each new instance of <code>NativeAdContent</code> to update the ad content.
///   </li>
///   <li>
...
///
/// \endcode\param size specifying the dimensions for displaying the native ad.
/// Use <code>AdSize.getInlineBanner()</code> to set any template size.
///
- (void)setAdTemplateSize:(CASSize * _Nonnull)size;
/// Configure registered native ad asset views with content from <code>NativeAdContent</code>.
/// Maps text, image, and media assets from the provided native ad to corresponding
/// asset views defined in this <code>CASNativeView</code>.
/// \param adContent Native ad content to display, or <code>nil</code> to clear all populated asset views.
///
- (void)bindAdContent:(CASNativeAdContent * _Nullable)adContent;
/// Configure registered native ad asset views with content from <code>NativeAdContent</code>.
/// Maps text, image, and media assets from the provided native ad to corresponding
/// asset views defined in this <code>CASNativeView</code>.
/// \param adContent Native ad content to display, or <code>nil</code> to clear all populated asset views.
///
/// \param autoPopulateText Whether text-based asset views should be populated automatically.
/// Disable this only when text assets are handled manually.
///
- (void)bindAdContent:(CASNativeAdContent * _Nullable)adContent autoPopulateText:(BOOL)autoPopulateText;
/// Configure registered native ad asset views with content from <code>NativeAdContent</code>.
/// Maps text, image, and media assets from the provided native ad to corresponding
/// asset views defined in this <code>CASNativeView</code>.
/// \param content The native ad content to display, or <code>nil</code> to clear the view.
///
- (void)setNativeAd:(CASNativeAdContent * _Nullable)content SWIFT_DEPRECATED_MSG("Renamed to bindAdContent(_:)", "bindAdContent:");
...
///
- (void)setNativeAd:(CASNativeAdContent * _Nonnull)ad templateSize:(CASSize * _Nonnull)templateSize SWIFT_DEPRECATED_MSG("To improve performance, please call setTemplateSize(_:) once to create layout and bindAdContent(_:) for each new NativeAdContent.");
@property (nonatomic, readonly, copy) NSArray<UIView *> * _Nonnull clickableViews;
@property (nonatomic, readonly) CGSize intrinsicContentSize;
- (void)didMoveToWindow;
@end
...
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull yangoAds;)
+ (NSString * _Nonnull)yangoAds SWIFT_WARN_UNUSED_RESULT;
...
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull verve;)
+ (NSString * _Nonnull)verve SWIFT_WARN_UNUSED_RESULT;
...
+ (enum CASSourceId)getAdapterId:(NSString * _Nonnull)net SWIFT_WARN_UNUSED_RESULT;
...
/// and metadata that describe the ad. You must pass an instance of this class to <code>CASNativeView.bindAdContent(_:)</code>
