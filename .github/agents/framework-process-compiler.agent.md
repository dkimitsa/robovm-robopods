...
- ALL YAML edits MUST use the `apply_patch` tool.
...
- NEVER guess paths. Read the bro-gen YAML directly at its exact path; do NOT use `rg` or any other discovery search to verify its presence. If `<moduleFolder>` cannot be resolved, report the error and stop.
...
3. Verify `<moduleFolder>/src/main/bro-gen/<framework_name>.yaml` exists (direct read, no search). If not: report the error and stop.

## Step 1: Compile & install
1. Run the single build step with `bash`:
...
2. **Build failed** → analyze the errors and pick exactly one branch:
   - The failure is a simple binding/YAML issue fixable by adjusting `<moduleFolder>/src/main/bro-gen/<framework_name>.yaml` (for example: name conflicts) → make ONE recovery edit with `apply_patch`, then go to Step 3.
